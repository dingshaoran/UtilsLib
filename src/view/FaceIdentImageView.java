...
public class FaceIdentImageView extends ImageView {
	...
	public FaceIdentImageView(final Context context) {

		super(context);
		init(null, 0);
	}

	public FaceIdentImageView(final Context context, final AttributeSet attrs) {

		super(context, attrs);
		init(attrs, 0);
	}

	public FaceIdentImageView(final Context context, final AttributeSet attrs, final int defStyle) {
	...
	protected void onDraw(final Canvas canvas) {
		...
		if ((now >= startTime) && (now < endTime)) {
			...
			for (int i = 0; i < 9; i++) {
				...
				final float interpolation = interpolator.getInterpolation((float) (now - startTime) / duration);
	...
	private void cropImage(final int originalImageWidth, final int originalImageHeight) {
		...
		if ((!mAutoFaceDetection && (mEndX == 0) && (mEndY == 0) && (mEndScale == 1)) || (originalImageWidth <= 0) || (originalImageHeight <= 0)) {
	...
	private void getDefaultOffsets(final float[] offsets, final float maxOffsetX, final float maxOffsetY) {
	...
	private void getFaceOffsets(final Face[] faces, final float[] offsets, final float scaleFactor, final float width, final float height, final float maxOffsetX, final float maxOffsetY) {
		...
		try {
			...
			final float scaledOffsetX = (midPoint.x * scaleFactor) - ((width - maxOffsetX) * FACE_POSITION_RATIO_X);

			final float scaledOffsetY = (midPoint.y * scaleFactor) - ((height - maxOffsetY) * FACE_POSITION_RATIO_Y);
	...
	private Matrix getOriginalMatrix(final int originalImageWidth, final int originalImageHeight) {
		...
		case CENTER:
			...
			matrix.postTranslate((frameWidth - originalImageWidth) / 2, (frameHeight - originalImageHeight) / 2);

			break;

		case CENTER_CROP:

			final float maxScaleFactor = Math.max(fitHorizontallyScaleFactor, fitVerticallyScaleFactor);
		...
		case MATRIX:
			...
			matrix.set(FaceIdentImageView.super.getImageMatrix());
	...
	private void init(final AttributeSet attrs, final int defStyle) {
		...
		final TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.FaceIdentImageView, defStyle, 0);

		mActivateDetectionFlags = typedArray.getInt(R.styleable.FaceIdentImageView_activate_face_detection, 0);
		mActivateMatrixFlags = typedArray.getInt(R.styleable.FaceIdentImageView_activate_matrix, 0);

		final boolean autoFaceDetection = typedArray.getBoolean(R.styleable.FaceIdentImageView_center_face, false);

		mStartScale = typedArray.getFloat(R.styleable.FaceIdentImageView_pre_scale, -1);
		mStartX = typedArray.getFloat(R.styleable.FaceIdentImageView_translate_x, 0);
		mStartY = typedArray.getFloat(R.styleable.FaceIdentImageView_translate_y, 0);

		mEndScale = typedArray.getFloat(R.styleable.FaceIdentImageView_scale, -1);
		mEndX = typedArray.getFloat(R.styleable.FaceIdentImageView_translate_x, 0);
		mEndY = typedArray.getFloat(R.styleable.FaceIdentImageView_translate_y, 0);

		mAnimationDuration = typedArray.getInt(R.styleable.FaceIdentImageView_animation_duration, 0);

		final int interpolatorId = typedArray.getResourceId(R.styleable.FaceIdentImageView_animation_interpolator, NO_ID);

		mCyclicAnimation = typedArray.getBoolean(R.styleable.FaceIdentImageView_cyclic_animation, false);
	...
	private static class DefaultMasaccioFaceDetector implements MasaccioFaceDetector {
		...
		private final Map<Bitmap, Face[]> mFacesMap = Collections.synchronizedMap(new WeakHashMap<Bitmap, Face[]>());
		...
		public Face[] process(final Bitmap bitmap) {
			...
			if (bitmap565 != null) {
				...
				final FaceDetector faceDetector = new FaceDetector(bitmap565.getWidth(), bitmap565.getHeight(), MAX_FACES);
		...
		private Bitmap convertTo565(final Bitmap origin) {
			...
			if ((bitmap.getWidth() & 0x1) != 0) {
				...
				bitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth() & ~0x1, bitmap.getHeight());
			}
	...
	private class CropRunnable implements Runnable {
		...
		public void run() {
			...
			if ((frameWidth <= 0) || (frameHeight <= 0)) {
				...
				FaceIdentImageView.super.setScaleType(mOriginalScaleType);
			...
			FaceIdentImageView.super.setScaleType(ScaleType.MATRIX);
			...
			if (detectedFaces != null) {
				...
				getFaceOffsets(detectedFaces, translateOffset, maxScaleFactor, newImageWidth, newImageHeight, maxOffsetX, maxOffsetY);

				matrix = new Matrix();
				matrix.setScale(maxScaleFactor, maxScaleFactor);
				matrix.postTranslate(-translateOffset[0], -translateOffset[1]);

				if (hasAllFlags(matrixFlag, FLAG_NO_FACE) && !hasAllFlags(matrixFlag, FLAG_IF_FACE)) {
			...
			if ((endScale < 0) && (endX == 0) && (endY == 0)) {
				...
				if (detectedFaces != null) {
					...
					getFaceOffsets(detectedFaces, scaledTranslateOffset, endScaleFactor, endImageWidth, endImageHeight, endImageWidth - frameWidth, endImageHeight - frameHeight);

				} else {

					getDefaultOffsets(scaledTranslateOffset, endImageWidth - frameWidth, endImageHeight - frameHeight);
				}

				endMatrix.setScale(endScaleFactor, endScaleFactor);
				endMatrix.postTranslate(-scaledTranslateOffset[0] + (Math.abs(endImageWidth - frameWidth) * endX), -scaledTranslateOffset[1] + (Math.abs(endImageHeight - frameHeight) * endY));
			}
			...
			if (mAnimationDuration > 0) {
				...
				if ((startScale < 0) && (startX == 0) && (startY == 0)) {
					...
					final float scaledOffsetX = ((newImageWidth * (scale - 1)) / 2) + translateOffset[0];

					final float scaledOffsetY = ((newImageHeight * (scale - 1)) / 2) + translateOffset[1];

					startMatrix.postTranslate(-scaledOffsetX + (Math.abs(startImageWidth - frameWidth) * startX), -scaledOffsetY + (Math.abs(startImageHeight - frameHeight) * startY));
				}
