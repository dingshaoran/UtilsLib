package com.xutils.bitmap.core;
...
public class AsyncDrawable<T extends View> extends Drawable {
    ...
    private final WeakReference<ImageViewUtils.BitmapLoadTask<T>> bitmapLoadTaskReference;

    private final Drawable baseDrawable;

    public AsyncDrawable(Drawable drawable, ImageViewUtils.BitmapLoadTask<T> bitmapWorkerTask) {
        if (bitmapWorkerTask == null) {
            throw new IllegalArgumentException("bitmapWorkerTask may not be null");
        }
        baseDrawable = drawable;
        bitmapLoadTaskReference = new WeakReference<ImageViewUtils.BitmapLoadTask<T>>(bitmapWorkerTask);
    }

    public ImageViewUtils.BitmapLoadTask<T> getBitmapWorkerTask() {
