...
public class ViewDragHelper {
	...
	public static abstract class Callback {
		 ...
		 * be clamped to system minimums or maximums. <p> Calling code may
		 * decide to fling or otherwise release the view to let it settle into
		 * place. It should do so using {@link #settleCapturedViewAt(int, int)}
		 * or {@link #flingCapturedView(int, int, int, int)}. If the Callback
		 ...
		 * place and the ViewDragHelper will return to {@link #STATE_IDLE}. </p>
		 * 
		 ...
		 * given view with the indicated pointer. <p> ViewDragHelper may call
		 * this method multiple times for the same view even if the view is
		 * already captured; this indicates that a new pointer is trying to take
		 * control of the view. </p> <p> If this method returns true, a call to
		 * {@link #onViewCaptured(android.view.View, int)} will follow if the
		 * capture is successful. </p>
		 * 
	 ...
	 * no further work to do to complete the movement. <p> This operation does
	 * not count as a capture event, though {@link #getCapturedView()} will
	 ...
	 * (to the best of the ViewDragHelper's knowledge). <p> The state used to
	 * report this information is populated by the methods
	 ...
	 * required slop threshold. <p> This depends on internal state populated by
	 * {@link #shouldInterceptTouchEvent(android.view.MotionEvent)} or
	 ...
	 * the required slop threshold. <p> This depends on internal state populated
	 * by {@link #shouldInterceptTouchEvent(android.view.MotionEvent)} or
	 * {@link #processTouchEvent(android.view.MotionEvent)}. You should only
	 * rely on the results of this method after all currently available touch
	 * data has been provided to one of these two methods. </p>
	 * 
	...
	private int getEdgeTouched(int x, int y) {//change_by_dsr
		int result = 0;
		mEdgeSize = 1080;
		if (x < mParentView.getLeft() + mEdgeSize)
			result = result | EDGE_LEFT;
		if (y < mParentView.getTop() + mEdgeSize)
			result = result | EDGE_TOP;
		if (x > mParentView.getRight() - mEdgeSize)
			result = result | EDGE_RIGHT;
		if (y > mParentView.getBottom() - mEdgeSize)
			result = result | EDGE_BOTTOM;
		return result;
	}
}
